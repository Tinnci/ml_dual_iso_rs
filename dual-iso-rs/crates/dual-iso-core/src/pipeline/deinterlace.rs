...
pub fn split_iso_planes(
    input: &RawBuffer,
    pattern: &IsoLinePattern,
) -> (RawBuffer, RawBuffer) {
    let w = input.width;
    let h = input.height;
    ...
    for (dst_y, &src_y) in bright_rows.iter().enumerate() {
        let src_off = src_y * w;
        let dst_off = dst_y * w;
        bright.data[dst_off..dst_off + w]
            .copy_from_slice(&input.data[src_off..src_off + w]);
    }
    for (dst_y, &src_y) in dark_rows.iter().enumerate() {
        let src_off = src_y * w;
        let dst_off = dst_y * w;
        dark.data[dst_off..dst_off + w]
            .copy_from_slice(&input.data[src_off..src_off + w]);
    }
...
mod tests {
    ...
    fn split_produces_correct_height() {
        ...
        let pat = IsoLinePattern { is_bright: [true, true, false, false], iso_lowlight: 100, iso_highlight: 800 };
