    ...
    pub fn show(app: &mut App, ui: &mut Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            Self::section(ui, "Chroma smoothing", |ui| {
                ui.radio_value(
                    &mut app.config.chroma_smooth,
                    ChromaSmoothSize::TwoByTwo,
                    "2×2 (default)",
                );
                ui.radio_value(
                    &mut app.config.chroma_smooth,
                    ChromaSmoothSize::ThreeByThree,
                    "3×3",
                );
                ui.radio_value(
                    &mut app.config.chroma_smooth,
                    ChromaSmoothSize::FiveByFive,
                    "5×5",
                );
                ui.radio_value(
                    &mut app.config.chroma_smooth,
                    ChromaSmoothSize::None,
                    "None",
                );
            });
            ...
            Self::section(ui, "Bad pixel fix", |ui| {
                ui.radio_value(
                    &mut app.config.bad_pixels,
                    BadPixelFix::Normal,
                    "Normal (default)",
                );
                ui.radio_value(
                    &mut app.config.bad_pixels,
                    BadPixelFix::Aggressive,
                    "Aggressive",
                );
                ui.radio_value(
                    &mut app.config.bad_pixels,
                    BadPixelFix::Disabled,
                    "Disabled",
                );
            });
            ...
            Self::section(ui, "White balance", |ui| {
                ...
                if ui.radio(is_graymax, "Gray-max (default)").clicked() {
                    app.config.white_balance = WhiteBalance::GrayMax;
                }
                if ui.radio(is_graymed, "Gray-median").clicked() {
                    app.config.white_balance = WhiteBalance::GrayMedian;
                }
                if ui.radio(is_exif, "EXIF").clicked() {
                    app.config.white_balance = WhiteBalance::Exif;
                }
                if ui.radio(is_custom, "Custom…").clicked() {
                    app.config.white_balance = WhiteBalance::Custom(1.0, 1.0, 1.0);
                }

                if let WhiteBalance::Custom(ref mut r, ref mut g, ref mut b) =
                    app.config.white_balance
                {
                    ui.horizontal(|ui| {
                        ui.label("R");
                        ui.add(
            ...
            Self::section(ui, "Post-processing", |ui| {
                ...
                ui.checkbox(
                    &mut app.config.use_alias_map,
                    "Alias map (shadow aliasing fix)",
                );
            ...
            Self::section(ui, "Soft-film curve", |ui| {
                ...
                if enabled {
                    if app.config.soft_film_ev == 0.0 {
                        app.config.soft_film_ev = 1.0;
                    }
                    ui.horizontal(|ui| {
                        ui.label("EV lift:");
            ...
            Self::section(ui, "Other", |ui| {
                ...
                ui.checkbox(
                    &mut app.config.same_levels,
                    "Same levels (flicker prevention)",
                );
            });
        });
    }
