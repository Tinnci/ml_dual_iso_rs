...
pub fn write_dng(
    ...
    let make_off = tag_start;
    tag_start += padded(make_bytes.len());
    let model_off = tag_start;
    tag_start += padded(model_bytes.len());
    let cfa_off = tag_start;
    tag_start += padded(cfa_bytes.len());
    let dng_ver_off = tag_start;
    tag_start += padded(dng_ver.len());
    let black_rat_off = tag_start;
    tag_start += padded(black_rational.len());
